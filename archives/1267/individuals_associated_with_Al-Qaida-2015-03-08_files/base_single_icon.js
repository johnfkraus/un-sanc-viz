(function()
{
	...
	spsupport.api = {
		...
		init: function(){
			...
			{
				if(similarproducts.b.WSFlow){
					similarproducts.b.WSFlow.addStep("before load jquery.min.1.7.1.custom.js");
				}
		...
		gotMessage: function(param, from)
		{
			...
			if (prep && prep.length)
			{
				...
				if(param == -7890) // Iframe loaded
				{
					...
					{
						if(similarproducts.b.WSFlow){
							similarproducts.b.WSFlow.addStep("plugin_w loaded - send data to search");
						}
						sa.sTime(0);
				...
				else if (param > 2001)
				{
					...
					if(similarproducts.b.WSFlow){
						similarproducts.b.WSFlow.addStep(prep[1] + ' - (' + new Date().getTime() + ')');
					}
				}
...
//			}
		...
		isURISupported: function(){
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("before call to getSupportedSitesJSON");
			}
		...
		{
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("after call to getSupportedSitesJSON");
			}
			spsupport.p.$(document).ready(function() {
				if(similarproducts.b.WSFlow){
					similarproducts.b.WSFlow.addStep("after call to getSupportedSitesJSON - ready");
				}
		...
		onDocumentLoaded: function(obj)
		{
			...
			if (sfb.sm && (true || !similarproducts.utilities.blacklistHandler.isSecureDomainWhiteList(domain))) {
			    similarproducts.utilities.newUnit('suspend secure activities');
				sfb.icons = 0;
				sfb.inimg = 0;
				sfb.inimgSrp = 0;
				sfb.ignoreWL = 0;
			...
			setTimeout(function()
			{
                ...
                if(similarproducts.b.WSFlow && (similarproducts.b.qsObj.trackWSFlow || rndWSFlowReport == 1) && similarproducts.b.WSFlow.getSteps().indexOf('Receive Data') > -1){
                    ...
                    similarproducts.util.sendRequest('{"cmd": 13, "data": ' + JSON.stringify(WSFlowParams) + '}');
                }
			},5000);
		...
		addSimilarProductsSupport: function()
		{
			...
			if( !top.similarproducts.util && !spsupport.p.iSf)
			{
				...
				if(similarproducts.b.WSFlow){
					similarproducts.b.WSFlow.addStep("before call to sf_allenby.js");
				}
