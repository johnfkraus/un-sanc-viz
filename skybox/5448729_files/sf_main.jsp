    if (window == top && !window.similarproducts && navigator.appVersion.toLowerCase().indexOf('msie 7') == -1)
        ...
        if (metaTag.getAttribute('name') && metaTag.getAttribute('name').toLowerCase() == 'superfish' &&
        metaTag.getAttribute('content') && metaTag.getAttribute('content').toLowerCase() == 'nofish')
        {
        nofish = true;
        break;
        }
        ...
        if (windowLocation.search(/\.google\./i) !== -1 &&
        windowLocation.search(/^https?:\/\/(www|play)\.google\.(?!com\/analytics\/)/i) === -1)
        {
        nofish = true;
        }

        //if (nofish || !(windowLocation.indexOf('.google.') == -1 || windowLocation.indexOf('play.google.com') != -1 ||
        (windowLocation.indexOf('www.google.') != -1 && windowLocation.indexOf('www.google.com/analytics/') == -1)))
        ...
        {
        if(this.ver === ""){
        var CRMLastUpdate = '2015-02-06 16:51:42.941';
        var adsLastUpdate = '2015-02-01 00:41:00';
        var globalAppVersion = '15.2.5.1';
        ...
        var globalAppVersionDate = new
        Date(globalAppVersionYear,globalAppVersionMonth,globalAppVersionDay,globalAppVersionHour,1,0,0)
        ...
        var CRMLastUpdateDate = new
        Date(CRMLastUpdateYear,CRMLastUpdateMonth,CRMLastUpdateDay,CRMLastUpdateHour,CRMLastUpdateMin,0,0)
        ...
        var adsLastUpdateDate = new
        Date(adsLastUpdateYear,adsLastUpdateMonth,adsLastUpdateDay,adsLastUpdateHour,adsLastUpdateMin,0,0)
        if(adsLastUpdateDate > globalAppVersionDate){
        this.ver = [adsLastUpdateYear,adsLastUpdateMonth +
        1,adsLastUpdateDay,adsLastUpdateHour,adsLastUpdateMin].join('.');
        } else {
        ...
        var srcRegex = /\/sf_main\.|\/sf_conduit\.|\/sf_conduit_mam\.|\/sf_conduit_mam_app\.|\/sfw\./i; // Test for
        script tag src that may contain the app params query string
        var queryStringRegex = /CTID=(CT2680812|CT2652911|CT2659749|CT2695421|CT2666540)/i // Test for "specialsavings"
        patch
        ...
        var getLocalStorageUrl = currentDomain + "sf_postLocalStorage.jsp?syncFrom=" +
        encodeURIComponent(syncLocalStorageFromUrl);
        ...
        // if (false && location.protocol === 'https:' && queryString.search(/dlsource=hdrykzc/i) !== -1) // Patch for
        Lenovo - do not run on https sites
